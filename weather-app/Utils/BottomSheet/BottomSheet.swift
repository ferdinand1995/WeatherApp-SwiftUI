...
public struct BottomSheetView<Header: View, Content: View, PositionEnum: RawRepresentable>: View where PositionEnum.RawValue == CGFloat, PositionEnum: CaseIterable, PositionEnum: Equatable {
    ...
    private var AnimationModel: AnimationModel = weather_app.AnimationModel(
...
extension BottomSheetView {
    public mutating func animationCurve(mass: Double = 1.2, stiffness: Double = 200, damping: Double = 25) -> BottomSheetView {
        let bottomSheetView = self
        AnimationModel = weather_app.AnimationModel(
            mass: mass,
            stiffness: stiffness,
            damping: damping
        )
